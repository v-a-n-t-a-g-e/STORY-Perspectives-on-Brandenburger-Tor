export default [
  // {
  //   name: "Gall 1",
  //   path: "models/Gall 1.ply",
  //   image: "images/Gall 1.jpg",
  //   fov: 43.66,
  //   transform: {
  //     position: [5.32, 1.0, 9.57],
  //     rotation: [-0.039, 0.04, -3.14],
  //     scale: [1, 1, 1],
  //   },
  // },
  {
    name: "Gall 2",
    path: "models/Gall 2.ply",
    image: "images/Gall 2.jpg",
    fov: 43.66,
    transform: {
      position: [70.82, 2.9, 2.47],
      rotation: [0.3408, 1.1466, -0.3405],
      scale: [2.61, 2.61, 2.61],
    },
  },
  {
    name: "Gall 3",
    path: "models/Gall 3.ply",
    image: "images/Gall 3.jpg",
    fov: 43.66,
    transform: {
      position: [46.25607457586088, 2.0664048757130455, -5.11073294029988],
      rotation: [2.042404853497039, 1.4865854087521844, -2.0665319581611694],
      scale: [2.23, 2.23, 2.23],
    },
  },
  {
    name: "Gall 4",
    path: "models/Gall 4.ply",
    image: "images/Gall 4.jpg",
    fov: 43.660659,
    transform: {
      position: [75.3561971428502, 2.651254436211598, -3.755262924245751],
      rotation: [-0.16719413258774793, 1.3712119922360861, 0.16393490079369008],
      scale: [2.04, 2.04, 2.04],
    },
  },
  // {
  //   name: "Gall 5",
  //   path: "models/Gall 5.ply",
  //   image: "images/Gall 5.jpg",
  //   fov: 43.679898,
  //   transform: {
  //     position: [81.816731366319, 2.86945993671436, -19.004039693550716],
  //     rotation: [0.6013845037086154, 1.4326352985052737, -0.5969254384819467],
  //     scale: [1.81, 1.81, 1.81],
  //   },
  // },
  // {
  //   name: "Gall 6",
  //   path: "models/Gall 6.ply",
  //   image: "images/Gall 6.jpg",
  //   fov: 43.679898,
  //   transform: {
  //     position: [16.77268939238746, 1.0000000000004095, -22.511120661269118],
  //     rotation: [2.4387747506346957, 1.1257243658916531, -2.5080191893095503],
  //     scale: [1.46, 1.46, 1.46],
  //   },
  // },
  // {
  //   name: "Säulenhalle Decke",
  //   path: "models/Säulenhalle Decke.ply",
  //   image: "images/Säulenhalle Decke.jpg",
  //   fov: 43.22837,
  //   transform: {
  //     position: [-29.981536611355637, 0.8000000000000681, 23.076471941522442],
  //     rotation: [1.1780253776398548, -1.290981559176644, 1.1638012614052615],
  //     scale: [1.67, 1.67, 1.67],
  //   },
  // },
  // {
  //   name: "Säulenhalle Innensäulen",
  //   path: "models/Säulenhalle Innensäulen.ply",
  //   image: "images/Säulenhalle Innensäulen.jpg",
  //   fov: 24.616032,
  //   transform: {
  //     position: [-28.19201895407154, 0.5999999999998437, 24.705276916647115],
  //     rotation: [2.8974459240641193, -0.8069142741496796, 2.9251997864012367],
  //     scale: [1, 1, 1],
  //   },
  // },
  // {
  //   name: "Säulenhalle Seite",
  //   path: "models/Säulenhalle Seite.ply",
  //   image: "images/Säulenhalle Seite.jpg",
  //   fov: 33.096605,
  //   transform: {
  //     position: [-19.086429085463013, 0.22429999999974182, 46.80030846711104],
  //     rotation: [
  //       0.3660541432364356, 0.016807677946267495, -0.05182080468966712,
  //     ],
  //     scale: [1.14, 1.14, 1.14],
  //   },
  // },
  // // {
  // //   name: "Kennedy 1963",
  // //   path: "models/Kennedy 1963.ply",
  // //   image: "images/Kennedy 1963.jpg",
  // //   fov: 46.808396,
  // //   transform: {
  // //     position: [-84.90858839834449, 5.599999999999741, -5.026112292179066],
  // //     rotation: [-0.7831462005838917, -1.3788502516483867, -0.7965692909228245],
  // //     scale: [2.02, 2.02, 2.02],
  // //   },
  // // },
  {
    name: "Kohl 1989",
    path: "models/Kohl 1989.ply",
    image: "images/Kohl 1989.jpg",
    fov: 43.602819,
    transform: {
      position: [10.468722331099398, 2.897999999821209, -4.13557138276874],
      rotation: [2.505015703966989, 1.3577876438119463, -2.515905728318489],
      scale: [1.8, 1.8, 1.8],
    },
  },
  {
    name: "Reagan 1987",
    path: "models/Reagan 1987.ply",
    image: "images/Reagan 1987.jpg",
    fov: 48.218611,
    transform: {
      position: [-65.6023063856759, 5.338664112051521, -0.5992502033155425],
      rotation: [0.61425880348939, -1.3350994697782634, 0.6046064930882524],
      scale: [1.95, 1.95, 1.95],
    },
  },
  {
    name: "Kennedy 1963",
    fov: 46.808396,
    path: "models/Kennedy 1963 alt.ply",
    image: "images/Kennedy 1963 alt.jpg",
    transform: {
      position: [-84.90858839834574, 6.487663337769723, -5.026112292179255],
      rotation: [-0.8899844683132758, -1.346118576174986, -0.9001925975259734],
      scale: [1.99, 1.99, 1.99],
    },
  },
];
